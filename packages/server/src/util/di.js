...
class DI {
    static provide(descriptor, ctor) {
        if (DI.ctors.has(descriptor))
            error(`descriptor ${descriptor} in use`);
    ...
    static inject(descriptor) {
        ...
        if (!DI.ctors.has(descriptor))
            error(`descriptor ${descriptor} not found`);
        if (stack.includes(descriptor)) {
            error('circular dependences: ' + stack.concat(descriptor).join(' > '));
        }
